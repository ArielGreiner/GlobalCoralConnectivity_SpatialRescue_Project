...
library(here) #added for plotting
...
#added for plotting:
id.totalnotseeded <- id.totalnotseeded[[((k-1)*1000 + j)]] #only one element of the list, so turn it into a non-list
...
#GOAL PLOT: Re-seeding from one random scenario, plotted on a map
png("GeneratingFigures_Code/Main Figures/Figure4b.png",width=20,height=20,units="cm",res=1500,pointsize=4) #res=1000
