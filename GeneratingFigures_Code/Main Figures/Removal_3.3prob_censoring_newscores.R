...
#didnt replot anything below here

# Select the BCUs - example
#id.50reefs <- which(!is.na(df[[i]]$BCU_Name))
#plot(world)
#points(df[[i]]$Longitude[id.50reefs],df[[i]]$Latitude[id.50reefs],col=cl[[i]]$membership[id.50reefs],pch=20,cex=0.2)

#map the 50 reefs in their clusters
reefdata1$cl.m <- clusters(g_orig, mode = "weak")$membership
#cols <- grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
#cols <- sample(tim.colors(clusters(g_orig, mode = "weak")$no)) #defined above instead
#reefdata1$cl.c <- cols[reefdata1$cl.m * 10]
reefdata1$cl.c <- cols[reefdata1$cl.m]
id.50reefs <- which(!is.na(reefdata1$BCU_Name))
png(file=paste0("50_reefs_3.3_", nrf_remaining[1],"%Reefs_Remaining.png"),width=10,height=10,units="cm",res=1000,pointsize=4)
plot(world, main = paste(nrf_remaining[1],"%Reefs Remaining"))
points(reefdata1$Longitude[id.50reefs],reefdata1$Latitude[id.50reefs],col=reefdata1$cl.c[id.50reefs],pch=20,cex=0.2)
dev.off()
dev.off()
...
df[[i]]$cl.c <- cols[df[[i]]$cl.m]
id.50reefs <- which(!is.na(df[[i]]$BCU_Name))
png(paste0("50_reefs_3.3_b.first_", nrf_remaining[i+1],"%Reefs_Remaining.png"),width=10,height=10,units="cm",res=1000,pointsize=4)
plot(world, main = paste("50 reefs", nrf_remaining[i+1],"%Reefs Remaining"))
points(df[[i]]$Longitude[id.50reefs],df[[i]]$Latitude[id.50reefs],col=df[[i]]$cl.c[id.50reefs],pch=20,cex=0.2)
dev.off()
}
dev.off()

#Remove the 50 reefs, what does the cluster distribution look like?
id.50reefs <- which(!is.na(reefdata$BCU_Name))
connmat_no50r <- connmat_reduced[-id.50reefs,-id.50reefs]
#dim(connmat_no50r) 8958x8958
reefdata1_no50r <- reefdata1[-id.50reefs,]
g_no50r <- graph.adjacency(as.matrix(connmat_no50r), weighted = TRUE)
cl_no50r <- clusters(g_no50r,mode="weak") #number of clusters, size of each cluster, membership of each cluster
dg_no50r <- degree(g_no50r) #gettting node degree
ss_no50r <- length(reefdata1$steppingstones[reefdata1$steppingstones == "yes"][-id.50reefs])
#note: g_orig <- graph.adjacency(as.matrix(connmat_reduced), weighted = TRUE)


#Number of clusters w/o 50reefs
cl_no50r$no #670 
#vs. clusters(g_orig, mode = "weak")$no 805 clusters

#Size of the largest cluster w/o 50 reefs
max(cl_no50r$csize) #956
#vs. max(clusters(g_orig, mode = "weak")$csize) #1121

#ignoring these two
#Plot what the distribution of cluster sizes looks like w/o 50 reefs
hist(cl_no50r$csize, xlab = "Cluster Size", ylab = "Frequency", xlim = c(0,800), main = paste("No 50 Reefs"))
hist(clusters(g_orig, mode = "weak")$csize, xlab = "Cluster Size", ylab = "Frequency", xlim = c(0,1200), main = paste("No Reefs Lost"))

#What is the average node degree w/o 50 reefs
mean(dg_no50r) #8.07591
#vs. mean(degree(g_orig)) #16.7094
...
#map the clusters when no reefs removed
#add a column to the dataframe IDing which grid cells are in which clusters + then assigning colours 
reefdata1$cl.m <- clusters(g_orig, mode = "weak")$membership
#cols <- grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
#reefdata1$cl.c <- cols[reefdata1$cl.m * 10]
reefdata1$cl.c <- cols[reefdata1$cl.m]
png("No_reefs_removed.png")
plot(world, main = "No reefs removed")
points(reefdata1$Longitude,reefdata1$Latitude,col=reefdata1$cl.c,pch=20,cex=0.2)
dev.off()
dev.off()

#map the clusters when 50 reefs removed
#add a column to the dataframe IDing which grid cells are in which clusters + then assigning colours (now w/n forloop)
reefdata1_no50r$cl.m <- cl_no50r$membership
#cols <- grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)]
#reefdata1_no50r$cl.c <- cols[reefdata1_no50r$cl.m * 10]
reefdata1_no50r$cl.c <- cols[reefdata1_no50r$cl.m]
png(file="50_Reefs_Removed_3.3.png",width=10,height=10,units="cm",res=1000,pointsize=4)
plot(world, main = "50 Reefs Removed")
points(reefdata1_no50r$Longitude,reefdata1_no50r$Latitude,col=reefdata1_no50r$cl.c,pch=20,cex=0.2)
dev.off()
dev.off()

#Histograms of initial configuration
#size of clusters
png(file="InitialClusterSizeDist_3.3.png",width=10,height=10,units="cm",res=1000,pointsize=4)
hist(clusters(g_orig, mode = "weak")$csize, xlab = "Cluster Size", breaks = 70, ylab = "Frequency", xlim = c(0,1200), main = "Size of Clusters before Removal, 3.3%")
dev.off()
...
#break into multiple histograms
png(file="InitialClusterSizeDist_under200_3.3.png",width=20,height=10,units="cm",res=1000,pointsize=4)
hist(clusters(g_orig, mode = "weak")$csize[clusters(g_orig, mode = "weak")$csize < 200], xlab = "Cluster Size", breaks = seq(0,200, by = 5), ylab = "Frequency", xlim = c(0,200), main = "Cluster Sizes (under 200), 3.3%")
axis(1,at = seq(0,200, by = 5))
dev.off()

png(file="InitialClusterSizeDist_over200_3.3.png",width=20,height=10,units="cm",res=1000,pointsize=4)
hist(clusters(g_orig, mode = "weak")$csize[clusters(g_orig, mode = "weak")$csize >= 200], xlab = "Cluster Size", breaks = c(seq(200,400,by=50), seq(500,6000, by = 1000)), ylab = "Frequency", xlim = c(200,6000), main = "Cluster Sizes (under 200), 3.3%")
axis(1,at = c(seq(200,400,by=50), seq(500,6000, by = 1000)))
dev.off()

#just looks like a mess
hist(clusters(g_orig, mode = "weak")$csize, xlab = "Cluster Size", breaks = c(seq(0,100, by = 5), seq(150,400,by=50), seq(500,6000, by = 1000)), ylab = "Frequency", xlim = c(0,1200), main = "Size of Clusters before Removal, 3.3%")
axis(1,at = c(seq(0,100, by = 5), seq(150,400,by=50), seq(500,6000, by = 1000)))
...
quantile(degree(g_orig), c(0.05,0.25,0.5,0.75,0.95))
# 5% 25% 50% 75% 95% 
#  2   6   9  12  17 


#####Things I'm not plotting anymore
#Plot how the distribution of cluster sizes changes across removal 'time'

#I think that plotting the density is really misleading bc the number of clusters is changing and the number of grid cells and stuff - try histograms?
#max(clusters(g_orig, mode = "weak")$csize)
#for(i in 1:24){ #1107 is max #IF PUT BACK IN - SHOULD GO TO 20
#print(max(cl[[i]]$csize))}
par(mfrow=c(5,5))
hist(clusters(g_orig, mode = "weak")$csize, xlab = "Cluster Size", ylab = "Frequency", xlim = c(0,1200), main = paste(nrf_remaining[1],"Reefs Remaining"))
for(i in 1:num_iter){
hist(cl[[i]]$csize, xlab = "Cluster Size", ylab = "Frequency", xlim = c(0,1200), main = paste(nrf_remaining[i+1],"Reefs Remaining"))	
}

#Plot how the distribution of the node degrees changes across removal 'time'
#max(degree(g_orig)) #42
for(i in 1:num_iter){ #biggest is 42
	print(max(dg[[i]]))
}
par(mfrow=c(5,5))
hist(degree(g_orig), xlab = "Node Degree", ylab = "Frequency", xlim = c(0,45), main = paste(nrf_remaining[1],"Reefs Remaining"))
for(i in 1:num_iter){
hist(dg[[i]], xlab = "Node Degree", ylab = "Frequency", xlim = c(0,45), main = paste(nrf_remaining[i+1],"Reefs Remaining"))
}
